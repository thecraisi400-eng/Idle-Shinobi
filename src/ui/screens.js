...
export function homeScreen(state) {
  ...
  const hero = panel(`<div class="hero-card"><div class="hero-avatar" aria-hidden="true">★</div><div><p class="eyebrow">EL CAMPEÓN DEL PUEBLO</p><h2>Nivel ${state.player.level}</h2>${progressBar({ label: "Experiencia", value: state.player.experience, maximum: state.player.experienceRequired })}</div></div>`, "hero-card-panel");
  const stats = panel(`<div class="section-heading"><h2>Estadísticas</h2><button class="text-button" type="button" data-route="${ROUTES.HERO}">Ver héroe</button></div><div class="stats-grid">${statRow("VIDA", state.player.stats.health)}${statRow("ATAQUE", state.player.stats.attack)}${statRow("DEFENSA", state.player.stats.defense)}${statRow("VELOCIDAD", state.player.stats.speed)}</div>`);
...
export function heroScreen(state) {
  return shell("Héroe", state, ROUTES.HERO, `${panel(`<div class="hero-detail"><div class="hero-avatar hero-avatar--large" aria-hidden="true">★</div><div><p class="eyebrow">CLASE POR ELEGIR</p><h2>${state.player.name}</h2><p class="muted">Entrena tus estadísticas para dominar el ring.</p></div></div>`)}${panel(`<div class="section-heading"><h2>Atributos</h2><span class="point-chip">${state.player.statPoints} puntos</span></div><div class="stats-list">${statRow("Vida", state.player.stats.health)}${statRow("Ataque", state.player.stats.attack)}${statRow("Defensa", state.player.stats.defense)}${statRow("Velocidad", state.player.stats.speed)}${statRow("Crítico", `${state.player.stats.critical}%`, "gold-text")}</div>`)}${panel(`<h2>Próxima meta</h2><p class="muted">Gana tu primera lucha para obtener EXP y oro.</p>`)}`);
}
...
export function combatScreen(state) {
  return `<section class="combat-screen" aria-labelledby="combat-title"><header><button class="back-button" type="button" data-action="back" aria-label="Salir del combate">‹</button><p>COMBATE DE PRUEBA</p><button class="speed-button" type="button" disabled>×2</button></header><div class="combat-stage"><div class="fighter fighter--hero"><span class="fighter-art" aria-hidden="true">★</span><h2>${state.player.name}</h2>${progressBar({ label: "Vida", value: state.player.stats.health, maximum: state.player.stats.health, tone: "health" })}</div><p class="versus">VS</p><div class="fighter fighter--enemy"><span class="fighter-art" aria-hidden="true">◆</span><h2>El Martillo</h2>${progressBar({ label: "Vida", value: 120, maximum: 120, tone: "danger" })}</div></div><div class="combat-log"><h1 id="combat-title">La lucha estará disponible en el paso 5</h1><p>La interfaz de combate ya está preparada para conectarse al motor automático.</p></div></section>`;
}
